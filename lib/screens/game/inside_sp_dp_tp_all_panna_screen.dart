...
class InsideSPDPTPAllPannaScreen extends StatefulWidget {
  const InsideSPDPTPAllPannaScreen({
    super.key,
    required this.title,
  ...
  @override
  State<InsideSPDPTPAllPannaScreen> createState() =>
      _InsideSPDPTPAllPannaScreenState();
...
class _InsideSPDPTPAllPannaScreenState
    extends State<InsideSPDPTPAllPannaScreen> {
  int total = 0;
  ...
  // Dropdown choices for digit selection
  List<int> digitChoices = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0];
  int selectedDigit = 1; // Default selected value

  // Dropdown choices for SP-DP-TP selection
  List<String> spDpTpChoices = [
    "SINGLE PATTI",
    "DOUBLE PATTI",
    "TRIPPLE PATTI"
  ];
  String selectedSpDpTp = "SINGLE PATTI"; // Default selected value
  ...
  void showLotteryPopup(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (_) => BidFamilyPopup(
        ...
        title: widget.title,
        api: 'https://rmmatka.com/app/api/spdp-bids',
      ),
    );
  }
  ...
  Future<void> addBids({required int amount, required int digit}) async {
    ...
    final result = await getFamilyNumber(
        digit,
        selectedSpDpTp == 'SINGLE PATTI'
            ? 'single'
            : selectedSpDpTp == 'DOUBLE PATTI'
                ? 'double'
                : 'tripple',
        'https://rmmatka.com/app/api/get-spdpall-panna');
    if (result != null) {
      if (result.error) {
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<int>(
                                ...
                                items: digitChoices.map((int digit) {
                              ...
                              // Dropdown for SP-DP-TP selection
                              DropdownButtonFormField<String>(
                                value: selectedSpDpTp,
                                onChanged: (value) {
                                  setState(() {
                                    selectedSpDpTp = value!;
                                  });
                                },
                                items: spDpTpChoices.map((String spDpTp) {
            ...
            Column(
              children: List.generate(allBids.length, (index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    title: Text(
                      'Digit: ${bid.digit}, Patti: ${bid.type}, Amount: ${bid.amount}',
  ...
  Column _submit(BuildContext context) {
    return Column(
      children: [
        Container(
          width: 260,
