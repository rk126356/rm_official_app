...
class InsideSPMototrPanna extends StatefulWidget {
  const InsideSPMototrPanna({
    super.key,
    required this.title,
  ...
  @override
  State<InsideSPMototrPanna> createState() => _InsideSPMototrPannaState();
...
class _InsideSPMototrPannaState extends State<InsideSPMototrPanna> {
  int total = 0;
  ...
  Future<void> addBids({required int amount, required int digit}) async {
    ...
    final result = await getFamilyNumber(
        digit, 'sp_motor_panna', 'https://rmmatka.com/app/api/get-spdp-panna');
    if (result != null) {
      if (result.error) {
